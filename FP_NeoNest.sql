...
VALUES ('TR001', 'ITM005', 2), ('TR002', 'ITM011', 1), ('TR003', 'ITM008', 1), ('TR004', 'ITM004', 3), ('TR005', 'ITM005', 1), ('TR006', 'ITM006', 1), ('TR007', 'ITM009', 2), ('TR008', 'ITM008', 1), ('TR009', 'ITM010', 3), ('TR010', 'ITM012', 1), ('TR011', 'ITM002', 1), ('TR012', 'ITM003', 1), ('TR013', 'ITM004', 2), ('TR014', 'ITM005', 3), ('TR015', 'ITM007', 1), ('TR016', 'ITM005', 1);
...
SELECT
    employee.emp_name AS 'Name Employee',
    transaction.tr_id AS 'Transaction ID'
FROM transaction
...
WHERE
    membership.mbr_dateCreated > "2023-07-16" -- Agregate Query
SELECT *
FROM transaction
...
SELECT COUNT(customer.cst_id) AS 'Total Customer' FROM customer
...
SELECT
    COUNT(customer.cst_id) AS 'Total Customer'
FROM customer
WHERE customer.cst_id IN (
        SELECT
            transaction.customer_cst_id
        FROM transaction
        WHERE
            transaction.tr_id IN (
                SELECT
                    transaction_item.transaction_tr_id
                FROM
                    transaction_item
                WHERE
                    transaction_item.item_item_id IN (
                        SELECT
                            item.item_id
                        FROM
                            item
                        WHERE
                            item.item_name 'Chair'
                    )
            )
    );
